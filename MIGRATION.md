...
4. **Graph check.** Regenerate `generated/orders.md`; confirm no cycle, and confirm every study's grounding field matches its actual ancestry. **Done.** No cycle, 45 nodes. `s-causal-structure` reclassified `grounded` → `derived` on 2026-09-23 (Ramsey's ruling: grounding describes the whole causal chain a conclusion rests on, not just its final measurement step) — see `records/decisions.md`.
