...
4. **Graph check.** Regenerate `generated/orders.md`; confirm no cycle, and confirm every study's grounding field matches its actual ancestry. **Done, with one flag.** No cycle, 45 nodes. `s-causal-structure` claims `grounded` while presupposing a `derived` study for target selection — satisfies the written rule literally but is in tension with its spirit; logged in `records/decisions.md` rather than resolved unilaterally.
